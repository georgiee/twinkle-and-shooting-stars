...
export function run(svgElement) {
  console.log('run shooting stars');
  const shootingStars = createShootingStars();
  svgElement.appendChild(shootingStars);
}
...
function createShootingStars() {
  ...
  shootingStar = createShootingStar({ x: 100, y: 100, radius: 100, angle: 45, debug: DEBUG});
  group.appendChild(shootingStar);
  allShootingStars.push(shootingStar);

  shootingStar = createShootingStar({ x: 350, y: 100, radius: 50, angleStart: 270, angle: 90, debug: DEBUG});
  group.appendChild(shootingStar);
  allShootingStars.push(shootingStar);

  shootingStar = createShootingStar({ x: 550, y: 750, radius: 600, angleStart: -90, angle: -150, debug: DEBUG});
  group.appendChild(shootingStar);
  allShootingStars.push(shootingStar);
...
function createShootingStar({x, y, radius, angle, angleStart = 0, debug = false}) {
  ...
  shootingStarShape.setAttribute('r', SIZE_SHOOTING_STAR + '');
